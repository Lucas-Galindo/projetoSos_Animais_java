...
import sosanimais.com.example.app.controller.services.PessoaService;
...
public class FuncionarioService implements IDAO<Funcionario>{
    ...
    public boolean delete(Pessoa pessoa, Funcionario entidade) {
        ...
        return SingletonDB.getConnection().manipular("DELETE FROM funcionario WHERE func_matricula="+entidade.getMatricula());
    }
    ...
    public Funcionario get(Pessoa pessoa, int mat) {
        ...
        ResultSet resultSet=SingletonDB.getConnection().consultar(sql);
        try {
            if (resultSet.next()) {
